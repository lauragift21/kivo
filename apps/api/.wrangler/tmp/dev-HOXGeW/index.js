...
// .wrangler/tmp/bundle-FlDrYH/checked-fetch.js
...
// .wrangler/tmp/bundle-FlDrYH/middleware-loader.entry.ts
init_checked_fetch();
init_modules_watch_stub();

// .wrangler/tmp/bundle-FlDrYH/middleware-insertion-facade.js
init_checked_fetch();
init_modules_watch_stub();
...
function formatErrorResponse(error, requestId, isDevelopment = false) {
...
function createLogger(requestId, userId) {
  ...
  return {
    ...
    error: /* @__PURE__ */ __name((message, error, data) => log("error", message, {
      ...data,
      error: error ? {
        name: error.name,
        message: error.message,
...
var errorHandlerMiddleware = createMiddleware(async (c, next) => {
  ...
  try {
    ...
    return c.json(formatErrorResponse(error, requestId, isDevelopment), 500);
  }
});
...
<body>
  <div class="header">
    <div class="logo-section">
      <h1>${settings2.business_name || "Kivo"}</h1>
...
var ReminderScheduler = class {
  ...
  async sendReminder(invoiceId, reminderType) {
    try {
      ...
      const response = await fetch("https://api.resend.com/emails", {
        ...
        body: JSON.stringify({
          from: settings2?.email_from_name ? `${settings2.email_from_name} <${fromEmail}>` : fromEmail,
  ...
  getReminderHTML(invoice, settings2, publicUrl, reminderType) {
    const businessName = settings2?.business_name || "Kivo";
